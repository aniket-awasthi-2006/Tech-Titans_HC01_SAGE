...
export default function PatientDashboard() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    color: '#F9FAFB', fontSize: isMobile ? 13 : 14, outline: 'none',
  };
    ...
    <DashboardLayout title={`Hello, ${user?.name?.split(' ')[0]}`} subtitle="Your OPD status" requiredRole="patient">
      <div style={{ maxWidth: isMobile ? '100%' : 660, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: isMobile ? 14 : 20 }}>
        ...
        {myTokens.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              return (
                <div key={tok._id} className={tok.status === 'in-progress' ? 'animate-pulse-glow' : ''} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 12, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
                    ...
                    <span style={{ fontSize: isMobile ? 12 : 13, fontWeight: 600, color: '#9CA3AF' }}>
                      {isSelf ? 'You (Self)' : `${tok.patientName} — ${RELATIONSHIP_LABELS[tok.relationship || 'other']}`}
                    </span>
                    {tok.patientGender && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#6B7280', background: 'rgba(255,255,255,0.06)', padding: '2px 8px', borderRadius: 20, whiteSpace: 'nowrap' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', borderRadius: 14, overflow: 'hidden', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                      {[
                        ...
                        <div key={i} style={{ flex: 1, padding: '14px 8px', textAlign: 'center', borderRight: i < 2 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                          ...
                          <div style={{ fontSize: isMobile ? 10 : 11, color: '#6B7280', marginTop: 3 }}>{s.label}</div>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <button onClick={() => { setJoinForm(defaultJoinForm); setShowJoinModal(true); }} style={{ padding: isMobile ? '11px 10px' : '12px', borderRadius: 12, border: '1px dashed rgba(99,102,241,0.35)', background: 'rgba(99,102,241,0.06)', color: '#A5B4FC', fontSize: isMobile ? 13 : 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: isMobile ? '28px 16px' : '40px 32px', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 700, color: '#F9FAFB', marginBottom: 8 }}>Not in Queue</h2>
            ...
            <button onClick={() => { setJoinForm(defaultJoinForm); setShowJoinModal(true); }} style={{ padding: isMobile ? '12px 22px' : '14px 36px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #6366F1, #8B5CF6)', color: 'white', fontSize: isMobile ? 14 : 15, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 24px rgba(99,102,241,0.45)', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: isMobile ? '16px 14px' : '20px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: isMobile ? 'wrap' : 'nowrap', marginBottom: 14 }}>
            <h3 style={{ fontSize: isMobile ? 13 : 14, fontWeight: 700, color: '#F9FAFB', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {waiting.length === 0 ? (
            <p style={{ color: '#6B7280', fontSize: isMobile ? 12 : 13, textAlign: 'center', padding: '12px 0' }}>Queue is empty - walk straight in!</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {waiting.slice(0, 6).map((t, i) => {
                ...
                return (
                  <div key={t._id} style={{ display: 'grid', gridTemplateColumns: isMobile ? 'auto auto minmax(0,1fr)' : 'auto auto minmax(0,1fr) auto auto', alignItems: 'center', gap: isMobile ? 8 : 10, padding: isMobile ? '9px 9px' : '9px 12px', borderRadius: 10, background: isMe ? 'rgba(99,102,241,0.12)' : 'rgba(255,255,255,0.03)', border: isMe ? '1px solid rgba(99,102,241,0.3)' : '1px solid rgba(255,255,255,0.04)' }}>
                    <Hash size={isMobile ? 12 : 14} color={i === 0 ? '#A5B4FC' : '#6B7280'} />
                    <span style={{ fontSize: isMobile ? 14 : 15, fontWeight: 800, color: i === 0 ? '#A5B4FC' : '#6B7280', minWidth: isMobile ? 24 : 28 }}>{t.tokenNumber}</span>
                    <span style={{ fontSize: isMobile ? 12 : 13, color: '#9CA3AF', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {maskPatientName(t.patientName)}
                      {isMe && <span style={{ marginLeft: 6, fontSize: isMobile ? 9 : 10, color: '#818CF8', fontWeight: 700 }}>(you)</span>}
                    </span>
                    <span style={{ fontSize: isMobile ? 10 : 11, color: '#6B7280', display: 'flex', alignItems: 'center', gap: 4, justifySelf: 'end', gridColumn: isMobile ? '2 / 4' : 'auto' }}>
              ...
              {waiting.length > 6 && <p style={{ fontSize: isMobile ? 10 : 11, color: '#4B5563', textAlign: 'center', margin: '4px 0 0' }}>+{waiting.length - 6} more</p>}
            </div>
          )}
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: isMobile ? '16px 14px' : '20px 24px' }}>
          <h3 style={{ fontSize: isMobile ? 13 : 14, fontWeight: 700, color: '#F9FAFB', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
            <FileText size={15} color="#9CA3AF" /> Consultation History
          </h3>
          {history.length === 0 ? (
            <p style={{ color: '#6B7280', fontSize: isMobile ? 12 : 13, textAlign: 'center', padding: '12px 0' }}>No past consultations yet.</p>
          ) : (
              ...
              {history.slice(0, 5).map(c => (
                <div key={c._id} style={{ padding: isMobile ? '12px 11px' : '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: isMobile ? 'wrap' : 'nowrap', gap: 6, marginBottom: 8 }}>
                    <span style={{ fontSize: isMobile ? 10 : 11, color: '#6B7280', display: 'flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <div style={{ fontSize: isMobile ? 13 : 14, fontWeight: 600, color: '#E5E7EB', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Stethoscope size={13} color="#A5B4FC" /> {c.diagnosis}
                  </div>
                  <div style={{ fontSize: isMobile ? 12 : 13, color: '#9CA3AF', display: 'flex', alignItems: 'flex-start', gap: 6 }}>
                    <span style={{ marginTop: 2, flexShrink: 0 }}>💊</span>
                    <PrescriptionDisplay prescription={c.prescription} compact />
                  </div>
                  {c.notes && <div style={{ fontSize: isMobile ? 11 : 12, color: '#6B7280', marginTop: 4, fontStyle: 'italic', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {showJoinModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'center', padding: isMobile ? 12 : 20, overflowY: 'auto' }}
          ...
          <div style={{ background: '#13192B', border: '1px solid rgba(99,102,241,0.25)', borderRadius: isMobile ? 16 : 22, padding: isMobile ? '18px 14px' : '30px 26px', width: '100%', maxWidth: 500, marginTop: isMobile ? 12 : 0, maxHeight: isMobile ? 'calc(100dvh - 24px)' : 'none', overflowY: isMobile ? 'auto' : 'visible', boxShadow: '0 32px 80px rgba(0,0,0,0.6)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: isMobile ? 16 : 22 }}>
              <h2 style={{ fontSize: isMobile ? 17 : 19, fontWeight: 700, color: '#F9FAFB', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                <UserPlus size={isMobile ? 18 : 20} color="#6366F1" /> Join OPD Queue
              </h2>
              <button onClick={() => setShowJoinModal(false)} style={{ background: 'none', border: 'none', color: '#6B7280', cursor: 'pointer', fontSize: isMobile ? 20 : 22, padding: 0 }}>✕</button>
            </div>
            ...
            <form onSubmit={joinQueue} style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 12 : 14 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexDirection: isMobile ? 'column' : 'row' }}>
                  {[{ val: true, label: 'Myself', Icon: UserCircle }, { val: false, label: 'Family Member', Icon: Users }].map(({ val, label, Icon }) => (
                    <button key={String(val)} type="button"
                      ...
                      style={{ flex: 1, padding: isMobile ? '10px 8px' : '10px', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, border: joinForm.forSelf === val ? '1.5px solid rgba(99,102,241,0.7)' : '1px solid rgba(255,255,255,0.1)', background: joinForm.forSelf === val ? 'rgba(99,102,241,0.18)' : 'rgba(255,255,255,0.04)', color: joinForm.forSelf === val ? '#A5B4FC' : '#9CA3AF', fontSize: isMobile ? 12 : 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>
              ...
              <div style={{ display: 'flex', flexDirection: isMobile ? 'column-reverse' : 'row', gap: 10, marginTop: 2 }}>
                <button type="button" onClick={() => setShowJoinModal(false)} style={{ flex: 1, width: '100%', padding: isMobile ? '12px' : '13px', borderRadius: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#9CA3AF', fontSize: isMobile ? 13 : 14, fontWeight: 600, cursor: 'pointer' }}>
                  Cancel
                </button>
      ...
      {unavailableNotice && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: isMobile ? 13 : 15, color: '#9CA3AF', lineHeight: 1.7, marginBottom: 0 }}>
